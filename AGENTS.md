...
Главный skill `/build-product` — единственная точка общения с пользователем. Остальные агенты делегируются оркестратором по режиму из `.project/runtime.json` (см. [platforms/delegation/](platforms/delegation/)).
